...
class Run():
    ...
    def plotData(self):
        ...
        fig.suptitle(f'Plots of Abs vs time for {self.temp}C', fontsize=14)
    ...
    def fitdata(self):
        ...
        self.Ai240, self.kTot240, self.sAi240, self.skTot240 = fitabs(self.d240[c, :], axs[0], name=self.n240,
                                                                      runname=self.temp + '_240')
        axs[0].set_title(self.n240)
        axs[0].set_title('240 nm Abs vs time ')
        axs[0].set_xlabel('Time')
        axs[0].set_ylabel('Abs')
        c = (self.d290[:, 0] > self.start) & (self.d290[:, 0] < self.stop)
        self.Ai290, self.kTot290, self.sAi290, self.skTot290 = fitabs(self.d290[c, :], axs[1], name=self.n290,
                                                                      runname=self.temp + '_290')
        axs[1].set_title(self.n290)
        axs[1].set_title('290 nm Abs vs time ')
        axs[1].set_xlabel('Time')
        axs[1].set_ylabel('Abs')
        ...
        axs[2].set_xlabel('Time')
        axs[2].set_ylabel('Abs')
        fig.suptitle(f'Fits of Abs vs time for {self.temp}C', fontsize=14)
        fig.tight_layout(rect=[0, 0, 1, 0.95])
        plt.show()
    ...
    def getrates(self):
        ...
        print(self.temp, ' & ', r4(k1), ' & ', r4(k2), ' & ', r4(self.k3), ' & ', r4(k4), ' & ', r4(self.kTot240),
              ' & ', r4(sk1), ' & ', r4(sk2), ' & ', r4(self.sk3), ' & ', r4(sk4), ' & ', r4(self.skTot240), '\\\\')
...
def fitabs(run, ax, name, runname):
    ...
    ax.set_ylabel('Abs')
    ax.legend()
    # print(runname, ' & ', r6(Ainf), ' & ', r6(dA), ' & ', r6(k), ' & ', r6(sAinf), ' & ', r6(sdA),
    # ' & ', r6(sk), '\\\\')
...
def arrhenius(rates):
    ...
    k1, k2, k3 = rates[:, 1], rates[:, 2], rates[:, 3]

    fig, axs = plt.subplots(nrows=1, ncols=3, figsize=(14, 5))

    # Subplot function to reduce code repetition
    def plot_subplot(ax, x, y, title, ylabel, equation_label):
        ax.scatter(x, np.log(y), c='k')
        ax.set_title(title, fontsize=16)
        ax.set_ylabel(ylabel, fontsize=14)
        ax.set_xlabel('$\\frac{1}{N_Ak_BT}$', fontsize=14)

        coeffs, cov = np.polyfit(x, np.log(y), 1, cov=True)
        slope, intercept = coeffs
        ax.plot(x, np.polyval([slope, intercept], x), 'r-', label='Fitted line')

        # Dynamic equation placement
        equation_text = f'${equation_label} = {intercept:.2f} + {slope:.2f} \\frac{{1}}{{N_Ak_BT}}$'
        ax.text(0.95, 0.95, equation_text, transform=ax.transAxes, fontsize=14,
                verticalalignment='top', horizontalalignment='right')

        return cov, intercept, slope

    # Plot all subplots
    cov1, intercept1, slope1 = plot_subplot(axs[0], xvec, k1, '$\\ln{k_1}$ vs $\\frac{1}{N_Ak_BT}$', '$\\ln{k_1}$',
                                            '\\ln{k_1}')
    cov2, intercept2, slope2 = plot_subplot(axs[1], xvec, k2, '$\\ln{k_2}$ vs $\\frac{1}{N_Ak_BT}$', '$\\ln{k_2}$',
                                            '\\ln{k_2}')
    cov3, intercept3, slope3 = plot_subplot(axs[2], xvec, k3, '$\\ln{k_3}$ vs $\\frac{1}{N_Ak_BT}$', '$\\ln{k_3}$',
                                            '\\ln{k_3}')
    ...
    fig.suptitle('Plots of $k_i$ vs $\\frac{1}{N_Ak_BT}$', fontsize=22)
    fig.tight_layout()
    plt.show()

    # Print results for LaTeX table
    for i, (slope, intercept, cov) in enumerate(
            [(slope1, intercept1, cov1), (slope2, intercept2, cov2), (slope3, intercept3, cov3)], 1):
        berr, merr = np.sqrt(np.diag(cov))
        Aerr = np.exp(berr) * berr
        print(f'k_{i} & {r4(np.exp(intercept))} & {r4(-slope)} \\\\')
        print(f'k_{i} errors & {r4(Aerr)} & {r4(merr)} \\\\')
